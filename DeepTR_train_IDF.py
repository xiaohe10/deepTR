...
def getIDF(query_answers):
    ...
    for query_answer in query_answers:
        count+=1
        print 'get idf:',count, '/', alllength
        query_term_set = set(query_answer['query_terms'])
        answer_term_set = set(query_answer['answer_terms'])
        all_set = query_term_set | answer_term_set
        for item in all_set:
            if(item in idf):idf[item]+=1
            else: idf[item] = 1
    for key in idf:
        idf[key] = math.log(alllength/idf[key])
    return idf
threshold = 0.2
...
def train(datasize,model):
                ...
                for answer_term in query_answer['answer_terms']:
                    ...
                    try:
                        ...
                        if (similarity > threshold):
                            termweight += similarity*idf[query_term]*idf[answer_term]
                            termisvalid = True
                    except KeyError:
    ...
    for alpha in alphas:
        ...
        print(clf.coef_)
        write(clf.coef_.tolist(),"output/idf/%d_alphais_%s.in" % (datasize,str(alpha)))
    return clf.coef_
import json
